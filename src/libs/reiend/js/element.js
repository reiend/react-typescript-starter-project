...
import { toKebab, isFunctionInstance, isArrayInstance } from './helpers.js';
...
const Element = () => {
  ...
  return elementInput => {
    ...
    const appendChildren = elementChildren => {
      ...
      // for array elements
      if (isFunctionInstance(elementChildren))
        elementChildrenTemp = elementChildren();
      // extract array of elements created from one Element function
      else if (elementChildren.init)
        elementChildrenTemp = elementChildren.init();
      // for single element
    ...
    const appendChild = elementChild => {
      ...
      if (elementChildTemp.init === undefined)
        element.appendChild(elementChildTemp);
    ...
    const prependChild = elementChild => {
      ...
      if (elementChild.init === undefined)
        element.prependChild(elementChildTemp);
      ...
      if (queryName === undefined) {
        ...
        if (element) {
          if (element.className.includes(cleanQueryPunctuation(queryName))) {
            ...
            const trimQueryName = `${existingQueryName} ${cleanQueryPunctuation(
              queryName
            )} `;
            element.setAttribute(
          ...
          if (
            elements[0].className.includes(cleanQueryPunctuation(queryName))
          ) {
            elements.forEach(element => {
              element.setAttribute(
                'class',
            ...
            elements.forEach(
              (element = element => {
                element.setAttribute(
                  ...
                  `${
                    element.className && element.className
                  } ${cleanQueryPunctuation(queryName)} `
                );
              })
            );
          }
